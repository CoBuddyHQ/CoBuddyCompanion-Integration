...
project.addSourceFilesAtPaths(path.join(__dirname, '../src/screens/dashboard/*.tsx'));
...
for (const sourceFile of project.getSourceFiles()) {
  ...
  function getTranslationKey(text) {
    ...
    let key = slug;
    let counter = 1;
    while (enJson.dashboard[key] && enJson.dashboard[key] !== text) {
      key = `${slug}_${counter}`;
      counter++;
    }
    enJson.dashboard[key] = text;
...
// Write back en.json
// Sort dashboard keys alphabetically for neatness
const sortedDashboard = Object.keys(enJson.dashboard)
  .sort()
  .reduce((obj, key) => {
    obj[key] = enJson.dashboard[key];
    return obj;
  }, {});
enJson.dashboard = sortedDashboard;

fs.writeFileSync(enJsonPath, JSON.stringify(enJson, null, 2), 'utf8');

console.log(`\nSuccessfully migrated ${modifiedFiles} screens.`);
console.log(`Updated en.json with new dashboard keys.`);
